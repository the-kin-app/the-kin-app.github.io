...
(function () {
  ...
  // The scan URL is on the WORKER's host, not the site's. kinapp.social is
  ...
  // lives at api.kinapp.social (worker/wrangler.toml `routes`), counts the
  // hit, and 302s to kinapp.social/waitlist/?l=&p= itself.
  const API = 'https://api.kinapp.social';
  const SITE = 'https://kinapp.social';
  ...
  function update() {
    ...
    } else if (/^https:\/\/api\.kinapp\.social\/[a-z0-9-]+\/[a-z0-9-]+$/.test(target.url)) {
      const [, l, p] = target.url.replace(`${API}/`, '/').split('/');
    ...
    } else if (/^https:\/\/kinapp\.social\/[a-z0-9-]+\/[a-z0-9-]+\/?$/.test(target.url)) {
      notes.push({ kind: 'bad', text: 'This looks like a tracking URL on the wrong host. kinapp.social is the ' +
        'static site — it will 404 and count nothing. Tracking lives on api.kinapp.social.' });
    } else {
